...
低吸策略完全独立于尾盘突破模型，使用 13 个低吸专用量价特征训练和推理，只有满足以下物理过滤时才调用低吸模型：

- `近5日最高涨幅 > 15.0`
- `今日急跌度 > -9.5`，拒绝濒临或封死跌停的飞刀
- `今日急跌度 < -4.0`
- `10日均线乖离率` 在 `-3.0` 到 `+3.0` 之间
- `今日缩量比例 < 0`

低吸模型仍预测真实 `next_day_premium`。不同于尾盘突破，`近3日断头铡刀标记` 不再篡改低吸训练标签，因为急跌本身就是低吸策略的入场条件之一。

生产过滤中，`首阴低吸` 保留 ST/退市、创业板/北交所/科创板、尾盘诱多等硬过滤，且在生成低吸物理池之前已经前置剔除 `30`、`68`、`4`、`8`、`92` 开头标的，保证低吸池只包含主板。低吸豁免 `上影线比例` 和 `近3日断头铡刀标记` 两个突破策略专用过滤，避免友军误伤。
...
尾盘突破模型中，如果一只股票过去 3 个交易日内出现过单日跌幅 `<= -7%`，训练集中会标记：
