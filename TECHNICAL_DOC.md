...
- `quant_core/data_pipeline/concept_engine.py` 负责 `data/concept_kline` 概念指数日线缓存、`data/concept_stock_map.json` 个股主概念映射、`data/concept_catalog.json` 概念目录。
- `quant_core/data_pipeline/sector_engine.py` 负责 `data/sector_kline` 一级行业指数日线缓存和 `stock_sector_map.parquet/csv` 个股行业兜底映射。
- `daily_factor_factory.py` 固定执行“细分概念优先，一级行业兜底，无数据释放 NaN (Theme Pipeline)”。
- `theme_*` 特征缺失时必须保留 NaN，禁止填 0，XGBoost 原生支持 NaN 分裂。
- 指数 K 线接口优先为 mpquant/Ashare 仓库的 `get_price(code, frequency='1d', count=..., end_date=...)`，也允许腾讯/新浪底层接口或本地合成缓存作为备用。
- `data/sector_kline/sector_index.parquet` 或 `.csv` 至少包含 `sector_name` 与 `sector_code`/`ashare_code`。
- `data/sector_kline/stock_sector_map.parquet` 或 `.csv` 至少包含 `code`、`sector_name`，建议同时保留 `name` 与 `sector_code`。
- 禁止新增或恢复 AkShare/东方财富板块链路。
...
│   └── sniper_status.json                 # Mac Sniper 前端保险匣共享状态
...
├── analyze_backtest_performance.py        # SQLite daily_picks 复利资金曲线分析脚本
...
├── sniper_status.py                       # data/sniper_status.json 共享开关读写
...
│   ├── daily_factor_factory.py            # V4.x Theme Alpha 全局日线模型因子工厂
│   ├── daily_model_trainer.py             # V4.x 全局日线 XGBoost 训练
│   ├── model_evaluator.py                 # V4.x 样本外评估
...
│   ├── global_momentum.py                 # 全局动量狙击，V4 Theme Alpha 波段策略
│   └── labs/
│       ├── strategy_lab.py
...
└── frontend/
    ...
    ├── src/App.vue                        # V5.6 Theme Alpha / Mac Sniper 工作站主界面
...
| `xgboost_daily_swing_global_v1.json` | 全局动量狙击 | 带 `theme_*` 因子的全市场 T+3 概率模型 |
...
- T-5 到 T-1 缩量蓄势，`contraction_amplitude_5d <= 12.0`。
- T 日量能引擎必须达到 `volume_burst_ratio >= 1.30`。
- T 日突破平台最高收盘价。
- T 日实体攻击，成交量温和放大。
...
- 先查 `concept_stock_map.json`，命中则读取 `data/concept_kline` 的概念指数日线。
- 未命中概念时查 `stock_sector_map`，读取 `data/sector_kline` 的一级行业指数日线。
- 两级都失败或 K 线缺失时，`theme_*` 显式保持 NaN，禁止填 0。
- 该策略归属波段/T+3 口径，收益字段复用 `t3_max_gain_pct`。

实盘硬风控：

- 只保留 `00`、`60` 开头主板股票，剔除创业板、科创板、北交所/三板。
- 名称中包含 `ST` 或 `*ST` 的股票一律剔除。
- 14:50 实时涨幅 `>= 9.0%` 的准涨停/涨停票一律剔除，避免实盘无法成交。
- 历史复盘保留同一类涨停不可交易拦截，但历史特征本身仍使用 15:00 完整日线，不再强行模拟 14:50 分时截面。

## 6. 多轨并行出票机制

当前生产逻辑不是全局唯一 Top1，也不再是每策略 Top3，而是“启用策略各取 Top1”：
...
- `prepare_historical_playback_candidates()` 和 `/api/backtest/top-pick-open` 使用 `stock_daily` 的日线完整行，`model_status` 应包含 `data_source=stock_daily_1500`。
...
生产分档逻辑位于 `quant_core/engine/predictor.py::select_strategy_top_picks()`：

```text
base 档：score >= MIN_SCORE，每个启用策略取 Top1，risk_warning 为空。
dynamic_floor 档：若 base 档为空，计算 dynamic_floor=max(0.55, legal_pool.score.quantile(0.99))，Top1 分数达到该动态底线才出票。
空仓：Top1 分数低于 dynamic_floor。
```

动态底线会在终端打印：

```text
[AdaptiveFloor] strategy=... legal_pool=... min_score=... dynamic_floor=... absolute_bottom=0.5500
```

仓位建议由 `calculate_suggested_position(probability, tier)` 统一计算：

- `base`：使用 `f = p - (1-p)/1.5` 的 Half-Kelly，结果乘以 `0.5` 后约束在 `10%` 到 `30%`。
- `dynamic_floor`：不看 Kelly，固定 `5%` 试错轻仓。
...
| `risk_warning` | 动态下探或历史导入的风偏提示 |
| `dynamic_floor` / `score_floor` | 当日合规池 99 分位动态底线 |
...
| 字段 | 说明 |
|---|---|
| `theme_name` / `theme_pct_chg_3` | 核心主题与 3 日主题动量 |
| `selection_tier` | 分档标签，`dynamic_floor` 表示逆势下探 |
| `risk_warning` | 风偏提示，前端与 PushPlus 必须醒目展示 |
| `dynamic_floor` / `score_floor` | 自适应动态底线 |
| `suggested_position` | 建议仓位比例 |

唯一性：

- 当前唯一索引为 `(selection_date, strategy_type, code)`。
- 不再允许假设 `selection_date` 或 `(selection_date, strategy_type)` 唯一。
- 历史 V3 全局狙击锁定记录存于 `v3_sniper_locks`。已确认旧数据未清除，可按原始快照价/时间导入 `daily_picks`，并在 `risk_warning` 中标记“未经过当前 V5.6 物理风控/动态底线重筛”。
...
```bash
cd /Users/eudis/ths
python3 scripts/backtest/simulate_sentinel_5m.py --start-date 2025-10-01 --end-date 2026-01-28
```
...
| 条件 | 正规军 | 敢死队/狙击/突破 |
|---|---:|---:|
| 盘中防爆止损 | `bar.low <= cost * 0.94` | `bar.low <= cost * 0.96` |
| 追踪止盈激活 | 最高价 `>= cost * 1.04` | 同左 |
| 追踪止盈触发 | 激活后 `bar.low <= highest * 0.98` | 同左 |
| 尾盘结构止损 | 14:50/14:55 `bar.close <= cost * 0.97` | 14:50/14:55 `bar.close <= cost * 0.985` |
| T+3 兜底 | T+3 最后一根 5m bar close 平仓 | 同左 |
...
### 8.5 Theme Pipeline：细分概念优先，一级行业兜底
...
- `concept_engine.py` 维护细分概念指数日线缓存，优先为个股提供主概念。
- `sector_engine.py` 维护一级行业指数日线缓存，只在概念映射缺失时兜底。
- `daily_factor_factory.py` 统一输出 `theme_pct_chg_1`、`theme_pct_chg_3`、`theme_volatility_5`、`rs_stock_vs_theme`、`rs_theme_ema_5`。
- 若概念和行业都缺失，或者对应指数 K 线读取失败，以上 `theme_*` 列必须保持 NaN，禁止 `fillna(0)`。
- 训练器和推理器都必须按 meta 的 `feature_columns` 顺序对齐，并保留 `theme_*` NaN 给 XGBoost。
...
- 读取 `stock_daily.close` 作为目标交易日 15:00 结算价，不再使用 14:45 盘中实时价做止盈、止损或追踪卖出。
- 结算收益按 `close_price / snapshot_price - 1` 计算，并调用 `mark_daily_pick_closed` 回填 `close_price`、`close_return_pct`、`close_signal`。
- 若目标日收盘价尚未入库，则保持持仓观察，不伪造结算。
...
- T+3 超时：持仓到期后按当前行情清仓并同步闭环。
- 分钟级爆量滞涨：腾讯 1m 分时最近 3 分钟均量超过全天均量 3 倍且价格不涨，推送【异动报警】；只报警，不移除持仓。
- 五档盘口委比反转：腾讯实时行情解析买一到买五、卖一到卖五挂单量，若 `weibi <= -80`，推送【盘口抢跑预警】；只报警，不移除持仓。

平仓同步：

- 任何防爆止损、追踪止盈、尾盘结构止损或 T+3 触发后都会调用 `mark_daily_pick_closed()`，把巡逻平仓价同步到 `daily_picks.close_price`、`close_return_pct`、`close_signal` 等结算字段。
...
GET /api/v4/sniper/history
```

V4 sniper 契约：

- `/api/v4/sniper/scan_today` 是 Theme Alpha 正式入口，`/api/v3/sniper/scan_today` 仅保留兼容。
...
POST /api/sniper/toggle
POST /api/sniper/test_fire
GET /api/shadow-account
POST /api/shadow-account/cash
POST /api/shadow-account/sync-broker
POST /api/shadow-account/test_order
```

说明：

- `/api/sniper/status` 和 `/api/sniper/toggle` 读写 `data/sniper_status.json`，这是 Web 服务、14:50 总线和巡逻兵共享的物理外挂保险匣。
- `/api/shadow-account` 读取 `data/shadow_account.json`，返回 `available_cash`、`locked_capital`、`total_shadow_equity`、`positions`、`trade_records` 和更新时间。
- `/api/shadow-account/cash` 覆盖设置影子资金池可用资金，立即影响下一次算股。
- `/api/shadow-account/sync-broker` 从同花顺交易页读取资金和持仓，同步为资金池基准。
- `/api/shadow-account/test_order` 支持 `execute=false` 算股预览和 `execute=true` 全自动休市试射。执行模式会先检查保险匣，再读取交易页当前价格，调用 `aim_and_fire(code, shares, limit_price)`。
- 本地成交流水只写入同花顺持仓确认后的 `broker_confirmed` 记录；券商弹窗、休市未成交、资金不足等状态只返回给前端，不写入“本地成交确认记录”。
...
```bash
cd /Users/eudis/ths
find data/cache -mindepth 1 -maxdepth 1 -exec rm -rf {} +
find data/strategy_cache -mindepth 1 -maxdepth 1 -exec rm -rf {} +
curl -sS "http://127.0.0.1:8000/api/backtest/top-pick-open?months=12&refresh=true"
```

Mac Sniper 与资金池烟测：

```bash
curl -sS http://127.0.0.1:8000/api/sniper/status
curl -sS http://127.0.0.1:8000/api/shadow-account
python3 -m py_compile quant_core/execution/mac_sniper.py quant_core/execution/position_sizer.py quant_core/sniper_status.py quant_dashboard/backend/main.py
```
