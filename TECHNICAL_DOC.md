# A 股量化工作站 V4.4 "Theme Alpha" 技术文档
...
- 当前为 V4.4 "Theme Alpha" 前向影子测试模式：四大核心军团各自独立分档出票，`全局动量狙击`、`右侧主升浪`、`中线超跌反转`、`尾盘突破` 每个策略基准线最多 Top3；若无基准线票，则按当日合规池 99 分位动态下探 1 只，并由实时巡逻兵风控接管。
- `首阴低吸` 保留代码和模型，但默认门槛 `99.00`，不进入生产出票。
- `quant_core/ai_agent` 本地 AI 右脑只做新闻/舆情/公告风险定性，不覆盖结构化模型排序。
- V4.4 生产输出必须携带 `selection_tier`、`risk_warning`、`dynamic_floor`、`suggested_position` 等字段，PushPlus、SQLite `raw_json` 和前端影子账本保持同一数据契约。
...
└── frontend/
    ...
    ├── src/App.vue                        # V4.4 Theme Alpha 工作站主界面
...
- 后续可继续做影子观察和失败归因。
- 当前 `PRODUCTION_OUTPUT_STRATEGIES` 只包含 `全局动量狙击`、`右侧主升浪`、`中线超跌反转`、`尾盘突破`。
...
- 该硬风控只作用于实盘出票链路，历史回测与模型训练不因此重写样本。

## 6. 多轨并行出票机制

当前生产逻辑不是全局唯一 Top1，而是四大核心军团独立分档出票：
...
6. `select_strategy_top_picks(limit_per_strategy=3)` 对每个策略执行分档录取，并避免同一股票重复入选多个策略。
7. `daily_picks` 通过唯一索引 `(selection_date, strategy_type, code)` 支持同策略同日多标的落库。
...
```text
base 档：score >= MIN_SCORE，取 Top3，risk_warning 为空。
...
- 历史 V3 全局狙击锁定记录存于 `v3_sniper_locks`。已确认旧数据未清除，可按原始快照价/时间导入 `daily_picks`，并在 `risk_warning` 中标记“未经过当前 V4.4 物理风控/动态底线重筛”。
...
- `中线超跌反转` / `右侧主升浪` / `全局动量狙击`：只处理 `< -4%` 的极端破位；其他情况静默洗盘，等待 14:45。
...
- 若无记录，则调用 `scan_market`，四大核心军团各自执行基准线 Top3 或动态下探 Top1。
...
- `/api/daily-picks` 返回前端影子账本，包含 `strategy_type`、`snapshot_price`、`open_premium`、`t3_max_gain_pct`、`theme_name`、`theme_pct_chg_3`、`selection_tier`、`risk_warning`、`suggested_position`、`is_closed`、`push_status` 等字段。
...
- 侧边栏：Dashboard、Shadow Test、单票行情库、Validation；旧 V3/V4 Command 独立页已下线。
- Dashboard：四大核心军团统一展示总预测、已结算次数、胜率、T+3/T+1 均值和持仓数，胜率只按已结算样本计算。
- 今日信号表统一融合 `daily_picks` 与全局狙击雷达结果，展示 `核心主题` 与 `主题3日动量`；`theme_pct_chg_3 > 3%` 时数字标红。
...
| `QUANT_GLOBAL_MIN_SCORE` | `0.85` | 全局动量狙击概率门槛 |
