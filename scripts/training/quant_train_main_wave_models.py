...
from build_main_wave_dataset import MAIN_WAVE_FEATURE_COLS, OUTPUT_PATH
from quant_core.config import BASE_DIR, MAIN_WAVE_MODEL_PATH


MODEL_PATH = MAIN_WAVE_MODEL_PATH
DATASET_PATH = OUTPUT_PATH
DRAWDOWN_PENALTY_PCT = -6.0
PENALIZED_TARGET_PCT = -3.0


def train() -> dict[str, Any]:
    df = pd.read_parquet(DATASET_PATH).sort_values(["date", "code"]).reset_index(drop=True)
    required_cols = [*MAIN_WAVE_FEATURE_COLS, "t3_max_gain_pct", "t3_max_drawdown_pct", "next_open_premium"]
    ...
    if len(df) < 500:
        raise RuntimeError(f"右侧主升浪训练样本过少: {len(df)}，不建议训练模型。")

    split_index = int(len(df) * 0.8)
    train_df = df.iloc[:split_index].copy()
    test_df = df.iloc[split_index:].copy()

    y_train_actual = train_df["t3_max_gain_pct"].clip(-6, 40)
    ...
    y_test_actual = test_df["t3_max_gain_pct"].clip(-6, 40)

    x_train = train_df[MAIN_WAVE_FEATURE_COLS]
    x_test = test_df[MAIN_WAVE_FEATURE_COLS]
    ...
    result = {
        ...
        "feature_count": len(MAIN_WAVE_FEATURE_COLS),
...
def format_training_report(result: dict[str, Any]) -> str:
    lines = [
        ...
        f"- 假突破惩罚：{result['penalty_rule']}，训练集中触发 {result['penalized_train_rows']} 条",
        ...
        "## 预测得分 Top 分层",
        "",
        "| 分层 | 实际T+3平均最大涨幅 | 胜率(T+3最大涨幅>0) | 平均最大回撤 |",
        "|---|---:|---:|---:|",
        f"| Top 10% | {result['top_10pct_actual_avg_t3_max_gain_pct']:.2f}% | {result['top_10pct_win_rate_pct']:.2f}% | {result['top_10pct_avg_drawdown_pct']:.2f}% |",
