...
def build_smart_dataset():
    ...
    for file in tqdm(all_files, desc="提取个股时序与量价特征"):
        ...
        ma5 = df_k['close'].rolling(5, min_periods=3).mean()
        ma10 = df_k['close'].rolling(10, min_periods=5).mean()
        ma20 = df_k['close'].rolling(20, min_periods=10).mean()
        df_k['5日均线乖离率'] = (df_k['close'] / ma5 - 1) * 100
        df_k['10日均线乖离率'] = (df_k['close'] / ma10 - 1) * 100
        df_k['20日均线乖离率'] = (df_k['close'] / ma20 - 1) * 100
        df_k['均线趋势斜率'] = (ma10 / ma10.shift(1) - 1) * 100
        df_k['光脚大阴线惩罚度'] = (df_k['close'] - df_k['low']) / (df_k['high'] - df_k['low'] + 1e-5)
